...
**General Purpose - T and M series** - For general purpose workloads(T-series - burstable, M-series - non burstable, normal general purpose)
...
Security Groups are traffic rules that you can define on services present inside VPC. It can be used to 
control inbound and outbound traffic on the following variables:
...
Route Table (fields): # no source as it's subnet level. Anyway you can configure more control at SG
  Destination (CIDR)
...
Physically attached storage attached to EC2 instance. Provides better performance at the cost of data loss(if instance is stopped), data is ephemeral(data loses if instance stops, terminates, or fails).
...
A Golden AMI is a ready-made machine image created by a central team. It already includes all updates, security fixes, and required software, so everyone can start from the same safe and trusted setup.

A normal AMI, on the other hand, is just a basic image. Each team may need to install updates, security patches, and software themselves, which can lead to differences and mistakes.
...
Use CloudFormation if:
  ...
  - You prefer managed state (no infra overhead)

Use Terraform if:
  - You want multi-cloud or hybrid setups
  - You need modularity and scalability
  - You prefer clean syntax and flexibility
  - You want to manage more than just infra (e.g., SaaS tools)
...
Scalability = On increasing the load, after what time does the system become unresponsive or slow. The lower the time, the more scalable is a system.
...
Managed redis and memcache service provided by AWS. Redis is more diverse, battle tested, single-threaded and covers many use cases. On the other hand, 
memcache is simple, multithreaded and provides more raw speed. Always go with redis unless you have a very specific use case in mind.
