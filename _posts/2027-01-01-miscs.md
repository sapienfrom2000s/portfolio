...
Liveness → Is the app still alive; if not restart
Readiness → Ready to Receive requests; remove endpoints

Q. Can you explain me vpc, subnet, security group, nat, igw, routing table and how they all are interrelated?
A. VPC is like a playground. It can have multiple smaller playgrounds inside it. Route table are routing rules
at subnet level. For eg- you might define these two rules:
0.0.0.0/0 -> IGW -> means all traffic should go to IGW
10.0.0.2/16 -> local -> this has higher specificity than 0.0.0.0 so if destination of the packet is in this
range, then the packet will be destined for local.
Routing table has no concept of inbound or outbound.
SG & NACL are applied at resource level, ie. maybe EC2.
